...
func getTestSwap() *cobra.Command {
	...
	testSwapCmd := &cobra.Command{
		Use:   "swap [requestTxHash] [source] [target] [amount] [requester] [destination] [tradeSlipLimit]",
		Short: "swap coins",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			targetPrice, err := types.NewAmount(args[6])
			if err != nil {
				return err
			}

			txHash, err := sb.SendSwap(txID, source, target, amt, args[4], args[5], clictx.GetFromAddress(), "welcome@1", "", targetPrice)
			if nil != err {
