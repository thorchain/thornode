...
func (s *BlockScannerTestSuite) TestSearchTxInABlockFromServer(c *C) {
  ...
  "result": {
	...
	bs, err := NewethereumBlockScanner(getConfigForTest(server.URL), 0, blockscanner.NewMockScannerStorage(), true, pv, s.m)
	c.Assert(err, IsNil)
	c.Assert(bs, NotNil)
...
func (s *BlockScannerTestSuite) TestGetTxHash(c *C) {
	...
	ethereumBlockScanner := ethereumBlockScanner{}
	obtainedTxHash, err := ethereumBlockScanner.getTxHash(encodedTx)
	c.Assert(err, IsNil)
...
func (s *BlockScannerTestSuite) TestFromTxToTxIn(c *C) {
	...
	testFunc := func(input string, txInItemCheck, errCheck Checker) []stypes.TxInItem {
		...
		pv := NewMockPoolAddressValidator()
		bs, err := NewethereumBlockScanner(getConfigForTest("127.0.0.1"), 0, blockscanner.NewMockScannerStorage(), true, pv, s.m)
		c.Assert(err, IsNil)
		c.Assert(bs, NotNil)
		c.Log(input)
	...
	// NewOrder Transaction on ethereum chain, THORNode don't care about it
	testFunc(ethereumTxNewOrder, IsNil, IsNil)

	// Normal tx send to our pool , withdraw
	testFunc(ethereumTxTransferWithdraw, NotNil, IsNil)
	// normal tx outbound from our pool
	testFunc(ethereumTxOutboundFromPool, NotNil, IsNil)
	txInItems := testFunc(ethereumTxOutboundFromPool1, NotNil, IsNil)
	...
	txInItems1 := testFunc(ethereumTxSwapLOKToBNB, NotNil, IsNil)
