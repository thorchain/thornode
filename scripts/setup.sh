...
while true; do
  ...
  thord add-genesis-account $(thorcli keys show statechain -a) 1000thor
  ...
  # add statechain as a trusted account
  {
    jq --arg SEQNO "$SEQNO" --arg VERSION "$(thorcli query swapservice version | jq -r .version)" --arg POOL_ADDRESS "$POOL_ADDRESS" --arg VALIDATOR "$(thord tendermint show-validator)" --arg NODE_ADDRESS "$(thorcli keys show statechain -a)" --arg OBSERVER_ADDRESS "$(thorcli keys show statechain -a)" '.app_state.swapservice.node_accounts[0] = {"node_address": $NODE_ADDRESS, "version": $VERSION, "status":"active","bond_address":$POOL_ADDRESS,"accounts":{"bnb_signer_acc": $POOL_ADDRESS, "bepv_validator_acc": $VALIDATOR, "bep_observer_acc": $OBSERVER_ADDRESS}} | .app_state.swapservice.pool_addresses.rotate_at="28800" | .app_state.swapservice.pool_addresses.rotate_window_open_at="27800" | .app_state.swapservice.pool_addresses.current[0] = {"chain":"BNB","seq_no":$SEQNO,"pub_key":$POOL_ADDRESS}'
