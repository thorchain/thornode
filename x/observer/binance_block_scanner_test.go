...
func (BlockScannerTestSuite) TestGetOneTxFromServer(c *C) {
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		c.Logf("testGetOneTxFromServer:%s", r.RequestURI)
		switch r.RequestURI {
		case "/api/v1/tx/22214C3567DCF0120DA779CC24089C8D18F9B8F217A5B1AD4821EFFFDD2BF92F?format=json":
			if _, err := w.Write([]byte(normalApiTx)); nil != err {
				c.Error(err)
			}
		case "/api/v1/tx/22214C3567DCF0120DA779CC24089C8D18F9B8F217A5B1AD4821EFFFDD2BF9XX?format=json":
			w.WriteHeader(http.StatusInternalServerError)
			return
		case "/api/v1/tx/22214C3567DCF0120DA779CC24089C8D18F9B8F217A5B1AD4821EFFFDD2BF9YY?format=json":
			if _, err := w.Write([]byte("whatever")); nil != err {
				c.Error(err)
			}
			return
		}
	...
	singleTxUrl1 := bs.getSingleTxUrl("22214C3567DCF0120DA779CC24089C8D18F9B8F217A5B1AD4821EFFFDD2BF9XX")

	bs.commonBlockScanner.GetHttpClient().Transport = trSkipVerify
	inItem1, err1 := bs.getOneTxFromServer("22214C3567DCF0120DA779CC24089C8D18F9B8F217A5B1AD4821EFFFDD2BF9XX", singleTxUrl1)
	c.Assert(err1, NotNil)
	c.Assert(inItem1, IsNil)

	singleTxUrl2 := bs.getSingleTxUrl("22214C3567DCF0120DA779CC24089C8D18F9B8F217A5B1AD4821EFFFDD2BF9YY")

	inItem2, err2 := bs.getOneTxFromServer("22214C3567DCF0120DA779CC24089C8D18F9B8F217A5B1AD4821EFFFDD2BF9YY", singleTxUrl2)
	c.Assert(err2, NotNil)
	c.Assert(inItem2, IsNil)
...
func (BlockScannerTestSuite) TestSearchTxInABlockFromServer(c *C) {
  ...
  "result": {
	...
	s := httptest.NewTLSServer(h)
	defer s.Close()
	bs, err := NewBinanceBlockScanner(getConfigForTest(s.Listener.Addr().String()), blockscanner.NewMockScannerStorage(), s.Listener.Addr().String(), common.BnbAddress("tbnb1ggdcyhk8rc7fgzp8wa2su220aclcggcsd94ye5"))
	c.Assert(err, IsNil)
	c.Assert(bs, NotNil)
	trSkipVerify := &http.Transport{
		MaxIdleConnsPerHost: 10,
