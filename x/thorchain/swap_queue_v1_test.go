...
func (s SwapQueueSuite) TestGetTodoNum(c *C) {
	queue := NewSwapQv1(KVStoreDummy{}, NewVersionedTxOutStoreDummy())
...
func (s SwapQueueSuite) TestScoreMsgs(c *C) {
	...
	pool.BalanceRune = sdk.NewUint(221793 * common.One)
	pool.BalanceAsset = sdk.NewUint(3 * common.One)
	c.Assert(k.SetPool(ctx, pool), IsNil)

	queue := NewSwapQv1(k, NewVersionedTxOutStoreDummy())
	...
	c.Check(swaps[5].msg.Tx.Coins[0].Amount.Equal(sdk.NewUint(100*common.One)), Equals, true, Commentf("%d", swaps[0].msg.Tx.Coins[0].Amount.Uint64()))
	c.Check(swaps[5].msg.Tx.Coins[0].Asset.Equals(common.BNBAsset), Equals, true)
	c.Check(swaps[6].msg.Tx.Coins[0].Amount.Equal(sdk.NewUint(50*common.One)), Equals, true, Commentf("%d", swaps[0].msg.Tx.Coins[0].Amount.Uint64()))
