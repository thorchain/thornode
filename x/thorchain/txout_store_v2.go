...
// TxOutStorageV2 is going to manage all the outgoing tx
type TxOutStorageV2 struct {
	height        int64
	keeper        Keeper
...
func NewTxOutStorageV2(keeper Keeper) *TxOutStorageV2 {
	return &TxOutStorageV2{
		keeper:  keeper,
		rwMutex: &sync.Mutex{},
	}
}
...
func (tos *TxOutStorageV2) NewBlock(height int64, constAccessor constants.ConstantValues) {
	tos.constAccessor = constAccessor
	tos.height = height
}

func (tos *TxOutStorageV2) GetBlockOut(ctx sdk.Context) (*TxOut, error) {
	return tos.keeper.GetTxOut(ctx, tos.height)
}

func (tos *TxOutStorageV2) GetOutboundItems(ctx sdk.Context) ([]*TxOutItem, error) {
	block, err := tos.keeper.GetTxOut(ctx, tos.height)
	return block.TxArray, err
}

func (tos *TxOutStorageV2) ClearOutboundItems(_ sdk.Context) {} // do nothing
...
func (tos *TxOutStorageV2) TryAddTxOutItem(ctx sdk.Context, toi *TxOutItem, eventManager EventManager) (bool, error) {
...
// fee deduction, etc
func (tos *TxOutStorageV2) UnSafeAddTxOutItem(ctx sdk.Context, toi *TxOutItem) error {
...
func (tos *TxOutStorageV2) prepareTxOutItem(ctx sdk.Context, toi *TxOutItem, eventManager EventManager) (bool, error) {
	...
	if err == nil && !memo.IsType(TxYggdrasilFund) && !memo.IsType(TxYggdrasilReturn) && !memo.IsType(TxMigrate) && !memo.IsType(TxRagnarok) {
		...
		if toi.Coin.Asset.IsRune() {
			...
			eventManager.UpdateEventFee(ctx, toi.InHash, fee)
...
func (tos *TxOutStorageV2) collectYggdrasilPools(ctx sdk.Context, tx ObservedTx, gasAsset common.Asset) (Vaults, error) {
	// collect yggdrasil pools
	var vaults Vaults
