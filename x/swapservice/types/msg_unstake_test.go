...
func (MsgUnstakeSuite) TestMsgUnstake(c *C) {
	...
	m := NewMsgSetUnStake(bnb, common.NewAmountFromFloat(100), common.BNBTicker, txID, acc1)
	EnsureMsgBasicCorrect(m, c)
	c.Check(m.Type(), Equals, "set_unstake")

	inputs := []struct {
		publicAddress       common.BnbAddress
		withdrawBasisPoints common.Amount
	...
	}{
		{
			...
			withdrawBasisPoints: common.NewAmountFromFloat(10000),
		...
		{
			publicAddress:       bnb,
			withdrawBasisPoints: common.NewAmountFromFloat(12000),
			ticker:              common.BNBTicker,
			requestTxHash:       txID,
			signer:              acc1,
		},
		{
			publicAddress:       bnb,
			withdrawBasisPoints: common.NewAmountFromFloat(-10),
			ticker:              common.BNBTicker,
			requestTxHash:       txID,
			signer:              acc1,
		},
