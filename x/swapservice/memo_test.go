...
func (s *MemoSuite) TestParse(c *C) {
	...
	c.Check(memo.IsCreate(), Equals, true, Commentf("MEMO: %+v", memo))

	memo, err = ParseMemo("STAKE:RUNE-1BA")
	c.Assert(err, IsNil)
	c.Check(memo.GetSymbol(), Equals, "RUNE-1BA")
	c.Check(memo.IsStake(), Equals, true, Commentf("MEMO: %+v", memo))

	memo, err = ParseMemo("WITHDRAW:RUNE-1BA:25")
	c.Assert(err, IsNil)
	c.Check(memo.GetSymbol(), Equals, "RUNE-1BA")
	c.Check(memo.IsWithraw(), Equals, true, Commentf("MEMO: %+v", memo))
	...
	c.Assert(err, IsNil)
	c.Check(memo.GetSymbol(), Equals, "RUNE-1BA")
	c.Check(memo.IsSwap(), Equals, true, Commentf("MEMO: %+v", memo))
