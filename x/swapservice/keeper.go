...
const (
	...
	prefixTxInIndex         dbPrefix = "txinIndex"
...
func (k Keeper) SetLastSignedHeight(ctx sdk.Context, height sdk.Uint) {
	...
	key := getKey(prefixLastSignedHeight, "", getVersion(ctx.BlockHeight(), prefixLastSignedHeight))
...
func (k Keeper) SetLastBinanceHeight(ctx sdk.Context, height sdk.Uint) error {
	...
	store := ctx.KVStore(k.storeKey)
	key := getKey(prefixLastBinanceHeight, "", getVersion(ctx.BlockHeight(), prefixLastBinanceHeight))
	store.Set([]byte(key), k.cdc.MustMarshalBinaryBare(height))
	return nil
}
...
func (k Keeper) GetPool(ctx sdk.Context, ticker common.Ticker) Pool {
	key := getKey(prefixPool, ticker.String(), getVersion(ctx.BlockHeight(), prefixPool))
	store := ctx.KVStore(k.storeKey)
	if !store.Has([]byte(key)) {
		return NewPool()
	}
...
func (k Keeper) SetPool(ctx sdk.Context, pool Pool) {
	store := ctx.KVStore(k.storeKey)
	key := getKey(prefixPool, pool.Ticker.String(), getVersion(ctx.BlockHeight(), prefixPool))
	if !store.Has([]byte(key)) {
...
func (k Keeper) GetStakerPool(ctx sdk.Context, stakerID common.BnbAddress) (StakerPool, error) {
	store := ctx.KVStore(k.storeKey)
	key := getKey(prefixStakerPool, stakerID.String(), getVersion(ctx.BlockHeight(), prefixPoolStaker))
...
func (k Keeper) SetTxInVoter(ctx sdk.Context, tx TxInVoter) {
	store := ctx.KVStore(k.storeKey)
	key := getKey(prefixTxIn, tx.Key().String(), getVersion(ctx.BlockHeight(), prefixTxIn))
...
func (k Keeper) GetTxInVoter(ctx sdk.Context, hash common.TxID) TxInVoter {
	key := getKey(prefixTxIn, hash.String(), getVersion(ctx.BlockHeight(), prefixTxIn))
...
func (k Keeper) GetTxInIndex(ctx sdk.Context, height uint64) (TxInIndex, error) {
	key := getKey(prefixTxInIndex, strconv.FormatUint(height, 10), getVersion(ctx.BlockHeight(), prefixTxInIndex))
	store := ctx.KVStore(k.storeKey)
	if !store.Has([]byte(key)) {
...
func (k Keeper) SetTxOut(ctx sdk.Context, blockOut *TxOut) {
	store := ctx.KVStore(k.storeKey)
	key := getKey(prefixTxOut, strconv.FormatUint(blockOut.Height, 10), getVersion(ctx.BlockHeight(), prefixTxOut))
...
func (k Keeper) GetTxOut(ctx sdk.Context, height uint64) (*TxOut, error) {
	store := ctx.KVStore(k.storeKey)
	key := getKey(prefixTxOut, strconv.FormatUint(height, 10), getVersion(ctx.BlockHeight(), prefixTxOut))
	if !store.Has([]byte(key)) {
...
func (k Keeper) SetAdminConfig(ctx sdk.Context, config AdminConfig) {
	store := ctx.KVStore(k.storeKey)
	key := getKey(prefixAdmin, config.DbKey(), getVersion(ctx.BlockHeight(), prefixAdmin))
...
func (k Keeper) GetIncompleteEvents(ctx sdk.Context) (Events, error) {
	key := getKey(prefixInCompleteEvents, "", getVersion(ctx.BlockHeight(), prefixInCompleteEvents))
	store := ctx.KVStore(k.storeKey)
	if !store.Has([]byte(key)) {
		return Events{}, nil
	}
...
func (k Keeper) GetCompletedEvent(ctx sdk.Context, id int64) (Event, error) {
	key := getKey(prefixCompleteEvent, fmt.Sprintf("%d", id), getVersion(ctx.BlockHeight(), prefixCompleteEvent))
	store := ctx.KVStore(k.storeKey)
	if !store.Has([]byte(key)) {
		return Event{}, nil
	}
...
func (k Keeper) SetCompletedEvent(ctx sdk.Context, event Event) {
	key := getKey(prefixCompleteEvent, fmt.Sprintf("%d", int64(event.ID.Float64())), getVersion(ctx.BlockHeight(), prefixCompleteEvent))
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetLastEventID(ctx sdk.Context) common.Amount {
	...
	key := getKey(prefixLastEventID, "", getVersion(ctx.BlockHeight(), prefixLastEventID))
	store := ctx.KVStore(k.storeKey)
	if store.Has([]byte(key)) {
		buf := store.Get([]byte(key))
