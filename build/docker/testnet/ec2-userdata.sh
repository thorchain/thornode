...
THORNODE_PASSWD=$(aws secretsmanager get-secret-value --secret-id ${THORNODE_ENV}-signer-passwd --region us-east-1  | jq -r .SecretString | awk -F'[:]' '{print $2}' | sed -e 's/}//' | sed -e 's/"//g')
...
start_stack () {
    cd $GIT_PATH/build/docker
    TAG=${THORNODE_ENV} && \
    BINANCE_HOST="http://testnet-binance.thorchain.info:26657" && \
    SIGNER_PASSWD=${THORNODE_PASSWD} && \
...
start_stack () {
    cd $GIT_PATH/build/docker
    BINANCE_HOST="http://testnet-binance.thorchain.info:26657" && \
    export PEER=$(curl -s http://${SEED_ENDPOINT}/${THORNODE_ENV}.json -L |tail -n 1 |jq '.'ip | sed -e 's/"//g' -e "s/null//g") && \
    SIGNER_PASSWD=${THORNODE_PASSWD} && \
