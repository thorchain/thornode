...
export SEED_ENDPOINT=https://seed.thorchain.info
export BUCKET_NAME=thorchain.info
export SEED_BUCKET="seed.${BUCKET_NAME}"
export S3_FILE="${THORNODE_ENV}_nodes.json"
export DATE=$(date +%F)
...
update_ip() {
echo "updating IP on https://seed.thorchain.info/testnet.json"
...
export PEER=$(curl -s ${SEED_ENDPOINT}/${THORNODE_ENV}.json |tail -n 1 |jq '.'ip | sed -e 's/"//g' -e "s/null//g")
